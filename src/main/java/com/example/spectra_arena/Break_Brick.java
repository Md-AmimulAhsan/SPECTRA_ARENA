...
public class Break_Brick {

    @FXML
    private Pane gamePaneZZ; // Link to game pane in FXML
    ...
    public void initialize() {
        ...
        gamePaneZZ.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.LEFT) {
                paddle.moveLeft(15); // Move paddle left
            } else if (event.getCode() == KeyCode.RIGHT) {
                paddle.moveRight(15, gamePaneZZ.getWidth()); // Move paddle right
        ...
        gamePaneZZ.setFocusTraversable(true);

        GoBackB.setFocusTraversable(false);
    ...
    private void checkCollisions() {
        ...
        if (ball.getX() <= 0 || ball.getX() >= gamePaneZZ.getWidth() - ball.getRadius() * 2) {
            ball.reverseX();
        }
        if (ball.getY() <= 0) {
            ball.reverseY();
    ...
    private void repaintGame() {
        // Clear the game pane
        gamePaneZZ.getChildren().clear();

        // Add the ball
        gamePaneZZ.getChildren().add(ball.getNode());

        // Add the paddle
        gamePaneZZ.getChildren().add(paddle.getNode());

        // Add visible bricks
        for (Brick brick : bricks) {
            if (!brick.isDestroyed()) {
                gamePaneZZ.getChildren().add(brick.getNode());
            }
        }
    }
    ...
    private boolean isGameOver() {
        ...
        return ball.getY() > gamePaneZZ.getHeight();
    }
